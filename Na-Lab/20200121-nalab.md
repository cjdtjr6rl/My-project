...
2. 게시판 선택<a id="select"></a>
   ...
   교수님이 사용하시는 게시판이 어떤 것이 있을까 고민을 하다가 일반 게시판과 과제 게시판으로 구분을 두었습니다.<br/>그 게시판들은 쓰기권한을 구분하여 게시판을 만든 관리자(교수) 제외 모든 사용자들이 못쓰게 만들거나, 모든 사용자들이 게시판을 쓰고 삭제할 수 있게 권한을 제공할 수 있습니다.<br/>
...
// 게시판 타겟 설정
   ...
   <br/>새로운 게시판일 경우 isNew가 true 이므로 radio 버튼이 전부 활성화가 되지만 기존에 있는 게시판은 false 이므로 name이 boardTypeRadio가 비활성화 됩니다.<br/>그리고 기존에 있는 게시판들은 DB에서 출력하여 그 게시판들의 데이터들이 지정되어 출력됩니다.
...
6. 정보 저장<a id="save"></a>
   ...
   impl에서 update된 게시판의 정보들이 배열의 값만큼 전달되어 수정됩니다.<br/>updateRoomBoard에서 모든 값을 가져와 조건에 맞춰 insert, update, delete하여 각각의 sql문에 들어가도록 구현하였습니다.<br/>
